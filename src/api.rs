...
mod tests {
    ...
    async fn test_submit_uses_internal_question_id() {
        ...
        // Expect submission to use internal question_id = 100, not frontend_question_id = 1
        Mock::given(method("POST"))
            .and(path("/problems/test-problem/submit/"))
            .and(wiremock::matchers::body_json(serde_json::json!({
                "lang": "rust",
                "question_id": "100",  // Should use internal ID
                "typed_code": "impl Solution { pub fn solve() {} }"
            })))
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_json(serde_json::json!({"submission_id": 12345i64})),
        ...
        let temp_dir = tempfile::tempdir().unwrap();
        let solution_file = temp_dir.path().join("solution.rs");
        std::fs::write(&solution_file, "impl Solution { pub fn solve() {} }").unwrap();

        // Submit by frontend_question_id = 1
        let result = client.submit(1, &solution_file).await;
        assert!(result.is_ok());
    }
